...
$result = saveRestaurantImage($_FILES['image_file']);
        ...
        if (empty($errors)) {
            ...
            if (file_exists($dbFile)) {
                ...
                if ($isEdit) {
                    $stmt = $db->prepare('UPDATE restaurant_items SET name = ?, description = ?, price = ?, category = ?, location = ?, contact = ?, information = ?, latitude = ?, longitude = ?, rating = ?, image = ? WHERE id = ?');
                    ...
                    $stmt = $db->prepare('INSERT INTO restaurant_items (name, description, price, category, location, contact, information, latitude, longitude, rating, image) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
                    $stmt->bindValue(1, $restaurant['name'], SQLITE3_TEXT);
                    $stmt->bindValue(2, $restaurant['description'], SQLITE3_TEXT);
                    $stmt->bindValue(3, $restaurant['price'], SQLITE3_TEXT);
                    $stmt->bindValue(4, $restaurant['category'], SQLITE3_TEXT);
...
<head>
    ...
    <title><?php echo $isEdit ? 'Edit' : 'Add'; ?> Restaurant - Tagum Admin</title>
    ...
    <header class="admin-header">
        <div class="admin-header-content">
            <div class="admin-title">
                <a href="dashboard.php?tab=restaurants" class="back-link">← Restaurants Dashboard</a>
